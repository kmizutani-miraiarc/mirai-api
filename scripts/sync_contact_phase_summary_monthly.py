#!/usr/bin/env python3
"""
HubSpotコンタクトのフェーズ集計バッチスクリプト（月次）
毎月末日に実行され、コンタクトの仕入フェーズ・販売フェーズを集計してDBに保存
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(log_dir, "contact_phase_summary_monthly.log"))
    ]
)
...
logger = logging.getLogger("contact_phase_summary_monthly")
...
class ContactPhaseSummaryMonthlySync:
    """コンタクトフェーズ集計クラス（月次）"""
    ...
    def _get_month_end_date(self) -> date:
        """前月末日の日付を取得（月初に実行されるため、前月末日を集計対象とする）"""
        today = date.today()
        # 前月の最終日を取得
        if today.month == 1:
            # 1月の場合は前年12月
    ...
    async def _save_to_database(
        ...
        async with self.db_pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cursor:
                # 既存のデータを削除（同じ集計日のデータ）
                delete_query = """
                    DELETE FROM contact_phase_summary_monthly
                ...
                try:
                    insert_query = """
                        INSERT INTO contact_phase_summary_monthly
                ...
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    for record_id, updated_contacts in record_updates.items():
                        try:
                            ...
                            update_query = """
                                UPDATE contact_phase_summary_monthly
                                SET contact_ids = %s
                                WHERE id = %s
                            """
...
async def main():
    sync = ContactPhaseSummaryMonthlySync()
    await sync.run()
